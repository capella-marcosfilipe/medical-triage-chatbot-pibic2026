...
class LangGraphRAGService:
    ...
    def __init__(self, manchester_rules: ManchesterRulesReader | None = None) -> None:
        self._checkpointer = MemorySaver()
        self._graph = self._build_graph()
        self._knowledge_chunks = self._load_knowledge_chunks(settings.RAG_KB_PATH)
        self._manchester_rules: ManchesterRulesReader = manchester_rules or ManchesterRulesRepository(
    ...
    def _retrieve_manchester_rules(self, query: str) -> list[Metadata]:
        ...
        missing, embedding error, etc.) is logged as a warning and treated
        as "no rules retrieved this turn" rather than a critical failure of
        the conversation.
        """
        try:
            return self._manchester_rules.query(text=query, top_k=settings.MANCHESTER_RAG_TOP_K)
        except Exception as exc:
            logger.warning(
                f"Manchester rules retrieval failed for this turn, continuing without it: {exc}. "
                "If the collection hasn't been built yet, run 'python -m app.rag.build_index'."
            )
            return []
